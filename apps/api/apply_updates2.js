...
async function run() {
  ...
  let id_operador = 'b9db434d-2e0f-4469-adf0-c9b03299d238';
  
  try {
      const uRes = await client.query('SELECT id FROM usuarios LIMIT 1');
      if(uRes.rows.length > 0) {
          id_operador = uRes.rows[0].id;
      }
  } catch(e) {}
  
  const armRes = await client.query("SELECT id FROM armazens WHERE nome ILIKE '%334%' OR codigo = '334' LIMIT 1");
  let armazem_id = '18795859-8bcf-4ebb-9b94-24e5a34e2452'; 
